...
public sealed class PlatformAudioController : IDisposable
{
    ...
    // The route is chosen once per session (when the mic opens); devices (dis)connecting
    // mid-conversation are picked up on the next location visit.
    ...
    public void Dispose()
    {
        ...
        // Undo the route override so the OS default applies again outside the call.
